...
Sixteen OWL-DL violation / reasoning cases covering issue #13 (six cases), plan-034
targeted OWL 2 DL parity verification (cases 7–14), and hasSelf + propertyDisjointWith
clash detection (cases 15–16) were run against native Konclude v0.7.0 and the WASM build
to classify discrepancies as port bugs vs upstream limitations.
...
| 15   | hasSelf(p) + hasSelf(q) + p owl:propertyDisjointWith q | inconsistent ✓ (expected) | inconsistent ✓ | **PARITY** (fixed by patch-022, 2026-09-16) |
| 16   | ReflexiveProperty(p) + hasSelf(q) + p owl:propertyDisjointWith q | inconsistent ✓ (expected) | inconsistent ✓ | **PARITY** (fixed by patch-022, 2026-09-16) |
...
WASM now correctly detects inconsistency for both constructs. Native Konclude v0.7.0
also detects them correctly (cases 15-16 are not upstream bugs; they were WASM port gaps).
...
Tests live in `tests/integration/issue13-owl-violations.test.ts` (cases 15-16).
