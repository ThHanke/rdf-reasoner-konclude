...
async function main() {
  ...
  const header = '| Ontology | Exp. | NTriples | Native parse ¹ | Native reasoning ² | WASM load ¹ | WASM classify ² | WASM total | TS total ³ | TS+expl ⁶ | Expl overhead | Enc (binary) ⁴ | Ratio ² | WASM inferred | TS inferred | Native inferred |';
  const sep    = '|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|';
  console.log(header);
  console.log(sep);
  ...
  for (const c of WASM_CASES) {
    ...
    let tsExpl = '—', explOverhead = '—';
    if (tc?.result?.ok && tc.result.explMs != null) {
      tsExpl = fmtMs(tc.result.explMs);
      explOverhead = `${tc.result.explOverhead}%`;
    }
    ...
    console.log(`| ${c.name} | ${exp} | ${nt} | ${nParse} | ${nClassify} | ${wLoad} | ${wClassify} | ${wTotal} | ${tsTotal} | ${tsExpl} | ${explOverhead} | ${encBinary} | ${ratioStr(wasmClassifyMs, nativeClassifyMs)} | ${wInferred} | ${tsInferred} | ${nInferred} |`);
  }
  ...
  console.log('⁶ "TS+expl" measures the same TS pipeline with `explanations: true` — includes bulk export from JustificationTripleCache + RDF-star serialization into N3 Store explanation graph. "Expl overhead" is the percentage increase over TS total.');
