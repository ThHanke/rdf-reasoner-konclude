...
const initPromise: Promise<KoncludeModule> = createKoncludeModule({ print: (msg: string) => self.postMessage({ type: "log", msg }), printErr: (msg: string) => self.postMessage({ type: "log", msg }) })
  .then((mod) => {
